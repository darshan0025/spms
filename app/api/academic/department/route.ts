...
export async function POST(req: Request) {
    ...
    await db.query("INSERT INTO department (department_name) VALUES (?)", [department_name]);
    return NextResponse.json({ message: "Department Added" });
}

export async function PUT(req: Request) {
    const { id, department_name } = await req.json();
    await db.query("UPDATE department SET department_name = ? WHERE id = ?", [department_name, id]);
